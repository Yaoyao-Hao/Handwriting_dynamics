%% LDR decoding  fig.4e (Kaufman et al.,2024)
...
% Vz
kin_dim = 3;
Vz = LDR_decode(stroke, stroke_kin_tmp, stroke_ind, stroke_aver, stroke_kin_aver_tmp, stroke_ind_aver, ...
                            penlift, penlift_kin_tmp, penlift_ind, penlift_aver, penlift_kin_aver_tmp, penlift_ind_aver, ...
                            dim, kin_dim,kin_basis_num);

% Grip
kin_dim = 4;
Grip = LDR_decode(stroke, stroke_kin_tmp, stroke_ind, stroke_aver, stroke_kin_aver_tmp, stroke_ind_aver, ...
                            penlift, penlift_kin_tmp, penlift_ind, penlift_aver, penlift_kin_aver_tmp, penlift_ind_aver, ...
                            dim, kin_dim,kin_basis_num);

% pressure
kin_dim = 5;
Pressure = LDR_decode(stroke, stroke_kin_tmp, stroke_ind, stroke_aver, stroke_kin_aver_tmp, stroke_ind_aver, ...
                            penlift, penlift_kin_tmp, penlift_ind, penlift_aver, penlift_kin_aver_tmp, penlift_ind_aver, ...
                            dim, kin_dim,kin_basis_num);
...
for sub=1:6
    ...
    EMG_sub = LDR_decode(stroke, stroke_kin_tmp, stroke_ind, stroke_aver, stroke_kin_aver_tmp, stroke_ind_aver, ...
                            penlift, penlift_kin_tmp, penlift_ind, penlift_aver, penlift_kin_aver_tmp, penlift_ind_aver, ...
                            dim, kin_dim,kin_basis_num);
...
function [B_dyn, spk_basisFxns, kin_basisFxns, VE] = LDR_train(spk_aver, kin_aver, ind_aver, neural_dim, kin_dim,kin_basis_dim)
    spk_aver(ind_aver) = [];
    kin_aver(ind_aver) = [];
    ...
    [kin_basisFxns, kin_act, ~] = eigTransform(kin_data, neural_dim); % extract movement dynamics from training dataset

    kin_approx = struct();
    for cond = 1:size(kin_act,2)
        kin_approx(cond).matrix = kin_act(cond).matrix*kin_basisFxns';
    end
    VE = getVarExplained(kin_data,kin_approx,'ind');
    
    kin_tr = struct_to_arr(kin_act, numel(kin_dim)*kin_basis_dim, numel(kin_act),[]);
    spk_tr = struct_to_arr(spk_act, 96*neural_dim, numel(spk_act),[]);

    lambda_vals = 1000;
    B_dyn = ridge_reg(spk_tr, kin_tr, lambda_vals); % ridge regress
end


function results = LDR_decode(stroke, stroke_kin, stroke_ind, stroke_aver, stroke_kin_aver, stroke_ind_aver, ...
...
for target = 1:30  % leave-one-character-out
    ...
    [B_stroke, spk_basis_s, kin_basis_s, ~] = LDR_train(stroke_aver, stroke_kin_aver, target_ind_s, dim, kin_dim,kin_basis_num);
    ...
    for i_stroke = 1:size(stroke_spk_tt, 2)
        if kin_dim==3
            ...
            pred_s = reshape(B_stroke' * stroke_spk_tt(:, i_stroke), numel(kin_dim), kin_basis_num) * kin_basis_s';
        end
    ...
    [B_penlift, spk_basis_p, kin_basis_p, ~] = LDR_train(penlift_aver, penlift_kin_aver, target_ind_p, dim, kin_dim,kin_basis_num);
    ...
    for i_penlift = 1:size(penlift_spk_tt, 2)
        if kin_dim==5
            ...
            pred_p = reshape(B_penlift' * penlift_spk_tt(:, i_penlift), numel(kin_dim), kin_basis_num) * kin_basis_p';
        end
